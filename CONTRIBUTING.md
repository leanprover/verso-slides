...
`scripts/test.sh test_tables.py -v` narrows down the browser tests.
...
[`pytest-xdist`](https://pytest-xdist.readthedocs.io/) using 4
workers by default. This gives roughly a 3–4× speedup over a
serial run without tripping over browser process contention.
...
- Override the worker count with the `PYTEST_WORKERS` env var:
  ...
  ```
- `PYTEST_WORKERS=auto` is flaky on machines with many cores
  because each worker launches its own Chromium; stick with a
  small integer unless you've verified your setup handles more.
...
CI must keep the default `--browser=all` so both browsers are
always exercised. For day-to-day local work, `--browser=random`
is a good compromise. Any single run is single-browser and fast,
but over time both browsers get exercised. There's no strong
performance reason to prefer one over the other locally.

### Running pytest Directly

`pytest` must be invoked from the `browser-tests/` directory
(that's where the `pyproject.toml` lives):
...
The session report header shows the resolved browser list, which
is useful for `--browser=random`:
...
changes to default rendering or to the executable's CLI), check
that `README.md` still describes the feature accurately.
